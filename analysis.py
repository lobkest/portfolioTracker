    ...
    for poging in range(1, pogingen + 1):
        try:
            ...
            if is_rate_limit and poging < pogingen:
                ...
                print(f"[classify] rate limited voor '{ticker}' (poging {poging}/{pogingen}), "
                      f"{wacht}s wachten...")
                time.sleep(wacht)
                continue
            print(f"[classify] ❌ kon info niet ophalen voor '{ticker}': {e}")
...
def classify_ticker(ticker):
    ...
    is_etf = _classify_ticker_uncached(ticker)
    if is_etf is None:
    ...
    save_classification(ticker, is_etf)
    return is_etf
...
def classify_tickers(tickers):
    ...
    for i, t in enumerate(te_doen):
        ...
        is_etf = _classify_ticker_uncached(t)
        if is_etf is None:
            result[t] = False  # niet cachen, volgende keer opnieuw proberen
        else:
            save_classification(t, is_etf)
            result[t] = is_etf
